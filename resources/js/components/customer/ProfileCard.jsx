...
export default function ProfileCard({ user, onEdit }) {
  ...
  const location = [user?.city, user?.country].filter(Boolean).join(', ') || 'No location added';
      ...
      <CardContent className="relative p-6">
        ...
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          ...
          <div className="min-w-0 flex-1 pr-10">
            <p className="-mt-1 text-xs font-semibold uppercase tracking-[0.24em] text-gray-500">Profile</p>
            <h2 className="mt-1 break-words text-3xl font-black tracking-[-0.03em] text-gray-800">{user?.name || 'Customer Name'}</h2>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <div className="flex items-start gap-3 rounded-2xl bg-gray-50 px-4 py-3">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500">Email</p>
                  <p className="mt-1 break-all text-sm font-medium text-gray-800">{user?.email || 'No email added'}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 rounded-2xl bg-gray-50 px-4 py-3">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500">Phone</p>
                  <p className="mt-1 text-sm font-medium text-gray-800">{user?.phone || 'No phone added'}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 rounded-2xl bg-gray-50 px-4 py-3 sm:col-span-2">
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-gray-500">Location</p>
                  <p className="mt-1 text-sm font-medium text-gray-800">{location}</p>
                  <p className="mt-1 text-sm text-gray-500">{user?.address || 'No address added'}</p>
                </div>
              </div>
            </div>
          </div>
